...
const markdown = `
### 서비스 이용약관
 **본 서비스의 이용약관의 목차는 아래와 같습니다.**

0. [제1조 - 목적](#chapter-0)
1. [제2조 - 정의](#chapter-1)
2. [제3조 - 약관 등의 명시와 설명 및 개정](#chapter-2)
3. [제4조 - 서비스의 제공](#chapter-3)
4. [제5조 - 서비스 이용 계약의 성립](#chapter-4)
5. [제6조 - 개인정보의 관리 및 보호](#chapter-5)
6. [제7조 - 서비스 이용계약의 종료](#chapter-6)
7. [제8조 - 회원에 대한 통지](#chapter-7) 
8. [제9조 - 저작권의 귀속](#chapter-8)
9. [제10조 - 게시물의 삭제 및 접근 차단](#chapter-9)
10. [제11조 - 광고의 게재 및 발신](#chapter-10)
11. [제12조 - 금지행위](#chapter-11)
12. [제13조 - 서비스 제공의 중단 및 서비스 이용 계약의 해지](#chapter-12)
13. [제14조 - 재판권 및 준거법](#chapter-13)
14. [제15조 - 기타 ](#chapter-14)

<a id="chapter-0"></a>

**제1조 - 목적** 
\`\`\`
 스쿨메이트 서비스 이용약관은 코디스트(Codest)(이하 "회사"라 합니다)가 제공하는 스쿨메이트 서비스의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임사항 등을 규정함을 목적으로 합니다.
\`\`\`
<a id="chapter-1"></a>
**제2조 - 정의**
...
\`\`\`
회사는 이 약관을 서비스 초기화면, 회원가입 화면 및 "내 정보" 메뉴 등에 게시하거나 기타의 방법으로 회원에게 공지합니다.
회사는 필요하다고 인정되는 경우, 관련법을 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.
회사는 약관을 개정할 경우, 적용 일자 및 개정사유를 명시하여 현행약관과 함께 개정약관 적용일자 7일 전부터 "공지사항"을 통해 공지합니다. 다만, 개정 내용이 회원의 권리 및 의무에 중대한 영향을 미치는 경우에는 적용일자 30일 전부터 회원의 연락처 또는 서비스 내부 알림 수단으로 개별 공지합니다.
회원은 개정 약관에 동의하지 않을 경우, 제7조(서비스 이용계약의 종료)에 따른 회원 탈퇴 방법으로 거부 의사를 표시할 수 있습니다. 단, 회사가 약관 개정 시 "개정 약관의 적용 일자까지 회원이 거부 의사를 표시하지 아니할 경우 약관의 개정에 동의한 것으로 간주한다"는 내용을 고지하였음에도 불구하고 회원이 약관 개정에 대한 거부의사를 표시하지 아니하면, 회사는 적용 일자부로 개정 약관에 동의한 것으로 간주합니다.
회원은 약관 일부분만을 동의 또는 거부할 수 없습니다.
회사는 제1항부터 제4항까지를 준수하였음에도 불구하고 회원이 약관 개정 사실을 알지 못함으로써 발생하는 피해에 대해 회사의 고의 또는 중대한 과실이 없는 한 어떠한 책임도 지지 않습니다. 
\`\`\`
<a id="chapter-3"></a>
...
\`\`\`
시간표, 버스 도착시간 등 학교 생활 편의 서비스
중학교/고등학교 별 폐쇄형 커뮤니티 서비스
진학, 문화, 활동, 수시 및 정시 정보 제공 서비스
할인, 이벤트, 프로모션, 광고 정보 제공 서비스
다른 회사 및 단체와의 제휴나 협력을 통해 제공하는 서비스
기타 회사가 정하는 서비스
\`\`\`
2. 스쿨메이트는 학생증, 학교 웹메일, 증명서, 성적표, 생기부 등을 통해 해당 중학교/고등학교의 재학생 또는 해당연도 입학생으로 인증받은 이용자에 한해 커뮤니티를 이용할 수 있는 폐쇄형 서비스입니다. 스쿨메이트는 중학교/고등학교 별로 개별 운영되므로, 학교 인증을 받지 않은 이용자 및 불특정 다수의 일반 이용자는 커뮤니티를 포함한 대부분의 기능에 접근할 수 없습니다.
...
6. 회사는 천재지변, 전쟁, 경영 악화 등 불가항력적인 사유로 인해 서비스 제공을 영구적으로 중단할 수 있습니다.
...
1. 회사와 회원의 서비스 이용계약은 서비스를 이용하고자 하는 자(이하 "가입신청자"라고 합니다)가 서비스 내부의 회원가입 양식에 따라 필요한 회원정보를 기입하고, 이 약관, 개인정보 수집 및 이용 동의 커뮤니티 이용규칙 등에 명시적인 동의를 한 후, 신청한 회원가입 의사 표시(이하 "이용신청"이라 합니다)를 회사가 승낙함으로써 체결됩니다.

2. 제1항의 승낙은 신청순서에 따라 순차적으로 처리되며, 회원가입의 성립 시기는 회사의 회원가입이 완료되었음을 알리는 승낙의 통지가 회원에게 도달하거나, 이에 준하는 권한이 회원에게 부여되는 시점으로 합니다.

3. 회사는 만 14세 미만 이용자의 경우 법적대리인의 허가를 필수적으로 요구하게 되어 있습니다.  가입 신청자는 이용신청 시 만 14세 이상에 해당한다는 항목에 명시적인 동의를 함으로써 회원은 만 14세 이상임을 진술하고 보증합니다.
...
가입 신청자가 만 14세 미만인 경우중 법정 대리인의 동의가 없는 경우
...
3. 회원의 아이디, 비밀번호, 이메일, 대학생 정보 등 모든 개인정보의 관리책임은 본인에게 있으므로, 타인에게 양도 및 대여할 수 없으며 유출되지 않도록 관리해야 합니다. 만약 본인의 아이디 및 비밀번호를 타인이 사용하고 있음을 인지했을 경우 즉시 문의 창구로 알려야 하고, 안내가 있는 경우 이에 따라야 합니다.
...
1. 회사는 유용하고 편리한 서비스를 제공하기 위해, 2023년부터 서비스 및 서비스 내부의 기능(시간표, 과제찾기, 버스 시간 안내 등)의 체계와 다양한 기능(검색 기능 등)을 직접 설계 및 운영하고 있는 데이터베이스 제작자에 해당합니다. 회사는 저작권법에 따라 데이터베이스 제작자는 복제권 및 전송권을 포함한 데이 터베이스 전부에 대한 권리를 가지고 있으며, 이는 법률에 따라 보호를 받는 대상입니다. 그러므로 이용자는 데이터베이스 제작자인 회사의 승인 없이 데이터베이스의 전부 또는 일부를 복제·배포·방송 또는 전송할 수 없습니다.

2. 회사가 작성한 게시물에 대한 권리는 회사에 귀속되며, 회원이 작성한 게시물에 대한 권리는 회원에게 귀속 됩니다.
...
1. 누구든지 게시물로 인해 사생활 침해나 명예훼손 등 권리가 침해된 경우 회사에 해당 게시물의 삭제 또는 반박내용의 게재를 요청할 수 있습니다. 이 때 회사는 해당 게시물을 삭제할 수 있으며, 만약 권리 침해 여부가 불분명하거나 당사자 간 다툼이 예상될 경우에는 해당 게시물에 대한 접근을 30일간 임시적으로 차단하는 조치를 취할 수 있습니다.
...
서비스, 브랜드, 사이트, 애플리케이션, 사업체, 단체 등을 알리거나 가입, 방문을 유도하기 위한 직·간 접적 홍보 행위
계정 판매 및 공유, 대리 게시, 서포터즈 등을 통해 여러 학교에 다발적으로 게시되는 동일한 주제에 대한 직간접적 홍보 행위
...
* 시스템 부정행위
\`\`\`
특정 중학교/고등학교 의 재학생 및 졸업생이 아닌 이용자가 해당 대학교의 재학생 및 졸업생인 것처 럼 기만하여 서비스를 이용하는 행위
프로그램, 스크립트, 봇을 이용한 서비스 접근 등 사람이 아닌 컴퓨팅 시스템을 통한 서비스 접근 행위
API 직접 호출, 유저 에이전트 조작, 패킷 캡처, 비정상적인 반복 조회 및 요청 등 허가하지 않은 방식 의 서비스 이용 행위
회사의 모든 재산에 대한 침해 행위
\`\`\`
* 업무방해 행위
\`\`\`
서비스 관리자 또는 이에 준하는 자격을 허가 없이 취득하여 권한을 행사하거나 사칭하여 허위의 정 보를 발설하는 행위
...
3. 이용자가 제1항에 해당하는 행위를 할 경우, 회사는 이 약관 제13조(서비스 제공의 중단 및 서비스 이용계약 의 해지)에 따라 서비스 제공을 중단하거나 서비스 이용계약을 해지할 수 있습니다.

<a id="chapter-12"></a>

**제13조 - 서비스 제공의 중단 및 서비스 이용계약의 해지**

1. 이용자가 이 약관 및 커뮤니티 이용규칙에서 이 조항 적용이 명시된 금지행위 및 이에 준하는 행위를 할 경우, 회사는 서비스 보호를 위해 다음과 같은 조치를 최대 영구적으로 취할 수 있습니다. 해당 조치는 서비스 보호를 위해 불가피하다고 판단될 경우, 회사가 제공하는 모든 서비스의 동일인으로 확인되는 모든 계정에 일괄적으로 적용될 수 있습니다.
\`\`\`
...
1. 이 약관은 2023년 11월 30일날 개정되었습니다. 

2. 이 약관에도 불구하고, 회사와 이용자가 이 약관의 내용과 다르게 합의한 사항이 있는 경우에는 해당 내용을 우선으로 합니다. 

3. 회사는 필요한 경우 약관의 하위 규정을 정할 수 있으며, 이 약관과 하위 규정이 상층하는 경우에는 이 약관의 내용이 우선 적용 됩니다. 
